...
/* REGOGER DATOS Y MOSTRAR */
    ...
    function getMoneda(id) {
        ...
        .then(function(json) {
            ...
            if(json.moneda.id) {
                ...
                alert('Error, la moneda ya no existe.');
            }
        })
        .catch(function(error) {
            document.body.classList.remove('body');
    ...
    function getPage(page) {
        fetch(page) // Peticion a la pagina
        .then(function(response) {
            return response.json(); // Si te llega algo lo conviertes en json
        })
        .then(function(json) {
            console.log(json);
            createTableDom(json);
            ...
            pageNumber = json.monedas.current_page; // Pagina en la que estamos ahora mismo
        })
        .catch(function(error) {
            alert('error');
            console.log('Request failed', error)
    ...
    getPage('ajaxmoneda'); // Al entrar a la pagina que se pidan los datos
    ...
    function createTableDom(jsonComplete) {
        deleteChildNodes('tbody'); // Borra si hay algo en la tabla
        let tbody = document.getElementById('tbody'); // Coge la tabla
        let json = jsonComplete.monedas; // Cogemos el json del getpage
    ...
    function createTd(text, classNames = [], data = [], id = '') {
        ...
        for (const dataAttribute of data) {
            td.dataset[dataAttribute.name] = dataAttribute.value; // uno por uno añade el data name con su valor
        }
        return td;
    }
